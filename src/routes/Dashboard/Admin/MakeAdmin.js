...
const MakeAdmin = () => {
    const { data: makeUserAdmin, isLoading, refetch } = useQuery('makeAdmin', () => fetch('https://dashboard.heroku.com/apps/mighty-taiga-34747/users').then(res => res.json()));

    const makeAnUserAdmin = (id) => {
        const url = `https://dashboard.heroku.com/apps/mighty-taiga-34747/user/${id}`;
