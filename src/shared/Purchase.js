...
const Purchase = () => {
    ...
    const { data: product } = useQuery('product', () => fetch(`https://dashboard.heroku.com/apps/mighty-taiga-34747/product/${id}`).then(res => res.json()))
    ...
    const handlePurchaseOrder = ({ prod }, event) => {
        ...
        const postUserOrder = async () => {
            const url = `https://dashboard.heroku.com/apps/mighty-taiga-34747/userOrder`;
