...
const Purchase = () => {
    ...
    const { data: product } = useQuery('product', () => fetch(`https://manufacturer-website-mw-server.onrender.com/product/${id}`).then(res => res.json()))
    ...
    const handlePurchaseOrder = ({ prod }, event) => {
        ...
        const postUserOrder = async () => {
            const url = `https://manufacturer-website-mw-server.onrender.com/userOrder`;
